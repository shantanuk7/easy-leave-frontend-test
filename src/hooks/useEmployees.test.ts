...
describe('useEmployees hook', () => {
  ...
  test('should fetch employees successfully', async () => {
    vi.spyOn(employeesApi, 'fetchEmployees').mockResolvedValue(mockPageResponse());
    const { result } = renderHook(() => useEmployees('2026'));
  ...
  test('should set hasMore false when last page', async () => {
    vi.spyOn(employeesApi, 'fetchEmployees').mockResolvedValue(mockPageResponse(true));
    const { result } = renderHook(() => useEmployees('2026'));
    await waitFor(() => expect(result.current.loading).toBe(false));
  ...
  test('should set hasMore true when more pages exist', async () => {
    vi.spyOn(employeesApi, 'fetchEmployees').mockResolvedValue(mockPageResponse(false));
    const { result } = renderHook(() => useEmployees('2026'));
    await waitFor(() => expect(result.current.loading).toBe(false));
  ...
  test('should not fetch when year is empty', () => {
    const spy = vi.spyOn(employeesApi, 'fetchEmployees');
  ...
  test('should reset employees when year changes', async () => {
    vi.spyOn(employeesApi, 'fetchEmployees').mockResolvedValue(mockPageResponse());
    const { result, rerender } = renderHook(({ year }) => useEmployees(year), {
  ...
  test('should set generic error when API fails with non-Error value', async () => {
    vi.spyOn(employeesApi, 'fetchEmployees').mockRejectedValue('unknown error');
    const { result } = renderHook(() => useEmployees('2026'));
    await waitFor(() => expect(result.current.loading).toBe(false));
